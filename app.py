...
if st.session_state.selected_project_code:
    if st.button("Close Project Hub", use_container_width=False):
        # Clear dictionary state to prevent automatic reopening
        if 'watchlist_grid' in st.session_state:
            st.session_state['watchlist_grid'] = {'selection': {'rows': [], 'columns': []}}
        if 'master_directory_grid' in st.session_state:
            st.session_state['master_directory_grid'] = {'selection': {'rows': [], 'columns': []}}
        go_to_dashboard()
        st.rerun()
        ...
        if projects_data:
            ...
            with dash_tab1:
                ...
                if not action_required_df.empty:
                    ...
                    if 'watchlist_grid' in st.session_state and len(st.session_state.watchlist_grid['selection']['rows']) > 0:
                        sel_idx = st.session_state.watchlist_grid['selection']['rows'][0]
            ...
            with dash_tab3:
                ...
                st.dataframe(
                    ...
                    selection_mode="single-row",
                    on_select="rerun",
                    key="master_directory_grid",
                    column_config=col_cfg_master
                )
                
                if 'master_directory_grid' in st.session_state and len(st.session_state.master_directory_grid['selection']['rows']) > 0:
                    sel_idx = st.session_state.master_directory_grid['selection']['rows'][0]
